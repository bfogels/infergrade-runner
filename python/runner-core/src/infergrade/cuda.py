...
from infergrade.runtimes import runtime_binary_fingerprint, windows_cuda_candidate_manifest
...
def _runtime_delivery_gate() -> Dict[str, Any]:
    ...
    role_map = {
        ...
        "cuda_runtime_dlls": "cuda_runtime_companion",
    }
    for artifact in list(candidate.get("artifacts") or []):
        if not isinstance(artifact, dict):
            continue
        url = str(artifact.get("url") or "").split("?", 1)[0].rstrip("/")
        name = url.rsplit("/", 1)[-1] if url else str(artifact.get("name") or "")
        if not name:
            continue
        artifacts.append(
            {
                "name": name,
                "role": role_map.get(str(artifact.get("role") or ""), artifact.get("role")),
