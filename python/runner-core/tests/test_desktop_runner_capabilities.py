...
class DesktopRunnerCapabilityTests(unittest.TestCase):
    ...
    def test_desktop_runner_surfaces_release_and_update_gates(self):
        ...
        self.assertIn('updateChannel.textContent = "Current release";', js)
    ...
    def test_desktop_docs_keep_native_first_and_docker_optional(self):
        ...
        self.assertIn("Docker will not be required for the first local benchmark", runner_readme)
        self.assertIn("Docker will not be required for the first local benchmark", desktop_readme)
        self.assertIn("Docker should not be required for the first local benchmark", first_user_path)
        self.assertIn("native first-run lane for macOS Apple Silicon", runner_readme)
        self.assertIn("recommended managed Metal runtime installed through the app", runner_readme)
        self.assertIn("checksum-verified rather than independently signed", runner_readme)
        self.assertIn("Docker/Podman: optional", desktop_readme)
        self.assertIn("Managed runtime install: explicit macOS Apple Silicon Metal lane available", desktop_readme)
        self.assertIn("broader channels and independent signature verification are planned", desktop_readme)
