...
class ReleaseCiTests(unittest.TestCase):
    ...
    def test_llama_cpp_intake_is_read_only_advisory_automation(self):
        ...
        self.assertIn("--model-canary-receipt runtime-intake/archive-receipts/legacy-control.json", workflow)
    ...
    def test_desktop_release_workflow_publishes_immutable_versioned_release_only_when_dispatched(self):
        workflow = (ROOT / ".github" / "workflows" / "desktop-runner-release.yml").read_text(encoding="utf-8")
        ...
        self.assertIn("Create or resume versioned draft release", workflow)
        self.assertIn("--draft", workflow)
        self.assertIn("--verify-tag", workflow)
        self.assertIn("already published and immutable", workflow)
        self.assertIn("Reconcile desktop release asset set", workflow)
        self.assertIn("gh release delete-asset", workflow)
        self.assertIn("Verify draft desktop release", workflow)
        self.assertIn("Publish immutable versioned desktop release", workflow)
        self.assertIn('gh release edit "$RELEASE_TAG" --draft=false --latest', workflow)
        self.assertLess(workflow.index("Verify draft desktop release"), workflow.index("Publish immutable versioned desktop release"))
        self.assertNotIn("RELEASE_TAG: desktop-runner-latest", workflow)
        self.assertIn("published without GitHub release immutability", workflow)
    ...
    def test_desktop_release_docs_match_protected_signing_and_notarization_gate(self):
        ...
        self.assertIn('identity_count" -ne 1', workflow)
        self.assertIn("Check desktop app dependencies", workflow)
        self.assertIn("Verify matching public runtime and capability images", workflow)
        self.assertIn('INFERGRADE_IMAGE_TAG="$DESKTOP_VERSION" ./scripts/verify_release_images.sh', workflow)
        self.assertIn('if [ "$DESKTOP_VERSION" != "$source_version" ]', workflow)
        ...
        self.assertIn("does not fall back to older capability images", docs)
