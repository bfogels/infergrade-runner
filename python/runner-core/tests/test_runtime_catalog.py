...
class RuntimeCatalogContractTests(unittest.TestCase):
    def test_projection_is_bound_to_signed_targets_and_exact_qualification(self):
        ...
        gemma4_12b_qualification = load_json(
        ...
        qualified_assertions = {
            ...
            for item in (
                ...
                + gemma4_12b_qualification["assertions"]
            )
        }
    ...
    def test_bonsai_candidate_is_active_without_overclaiming_compatibility(self):
        ...
        self.assertEqual(source["activation_status"], "active")
        self.assertEqual(source["versions"]["targets"], active_targets["signed"]["version"])
        self.assertIn(BONSAI_TARGET_NAME, active_targets["signed"]["targets"])
        ...
        self.assertEqual(
            qualification["runtime"]["signed_catalog_state"], "production_active"
        )
        self.assertEqual(
            qualification["runtime"]["catalog_targets_metadata_sha256"],
            hashlib.sha256(
                (REPO_ROOT / "runtime/catalog/signed/targets.json").read_bytes()
            ).hexdigest(),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            gemma4_qualification["runtime"]["catalog_targets_metadata_version"],
            active_targets["signed"]["version"],
        )
