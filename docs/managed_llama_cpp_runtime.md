...
- Upstream discovery is automated but promotion is not. The daily runtime-intake workflow reports new llama.cpp candidates and pin age without silently changing a user's selected runtime. For each exact upstream tag and verifier revision, it downloads the official macOS arm64, Linux x64, and Windows CPU x64 archives once, verifies GitHub's digest, bounded extraction, and expected tools, and caches those receipts. The Linux archive also gets a native `--version` smoke. Native macOS/Windows version smokes remain an explicit workflow dispatch, and all GGUF/model canaries remain separate evidence; an archive receipt is never model-compatibility proof.
