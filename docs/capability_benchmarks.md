...
That current/recent share and the three-band minimum do not by themselves prove the suite was challenged by a deliberately stressful model. Every surface must also include two observations of an explicit `headroom_challenge_eligible` campaign target, with the same exact setup independently repeated across two trusted evidence groups. Runner curates this role instead of inferring it from size or an observed high score, so future MoE or strong compact challengers can qualify without equating parameters with capability. The first declared target is the blocked Qwen3.6 27B canary. It cannot count until its artifact, fit, runtime, and prompt protocol are qualified, and its selection does not establish a frontier or leaderboard claim.
