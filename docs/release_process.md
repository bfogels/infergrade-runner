...
The release workflow accepts either Apple ID app-specific password notarization credentials or App Store Connect API-key credentials. The API-key lane uses `APPLE_API_KEY`, `APPLE_API_ISSUER`, and `APPLE_API_PRIVATE_KEY`; the workflow writes the private key into the runner temp directory as `APPLE_API_KEY_PATH` before invoking Tauri. The workflow imports `APPLE_CERTIFICATE` into an ephemeral keychain and requires exactly one valid Developer ID Application identity. It derives both the human-readable certificate name required by Tauri and the exact fingerprint used by direct `codesign` calls from that imported certificate instead of relying on a separately maintained identity string. It adds that keychain to the release job's user search list and explicitly unlocks it again at the signing-step boundary; the random keychain password is masked, job-scoped, and discarded with the keychain.
