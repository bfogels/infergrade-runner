...
Declared tier sampling is also insufficient by itself. `scripts/audit_benchmark_tiers.py --fail-invalid` materializes every native fixture and checks that canary, standard, and gold selections contain the catalog-required categories, structural tiers, tool outcomes, context lengths, and answer-key positions with explicit per-value case floors. It also rejects undersized fixtures, duplicate or missing task identities, and selection digests whose versioned serialization is absent or incompatible with the execution path. The first-party repository-edit fixture is checked through a bundled exact-source manifest and, in source checkouts, against the full container fixture SHA and ordered task/category manifest; its audited tier digests now use the same declared serialization as runtime metadata. Upstream container-owned datasets remain runtime-verified. These checks prevent a correctly labeled full fixture from silently producing a narrower positional shortcut, but passing structural coverage does not establish empirical difficulty, contamination resistance, or population representativeness.
