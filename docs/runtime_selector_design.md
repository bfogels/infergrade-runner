...
- `compatibility.reason_codes`: stable machine-readable codes. Initial codes should include `no_accelerator_detected`, `nvidia_smi_missing`, `driver_too_old`, `runtime_binary_missing`, `runtime_smoke_failed`, `checksum_failed`, `model_too_large`, `unsupported_model_architecture`, `container_runtime_missing`, and `fallback_not_allowed`.
